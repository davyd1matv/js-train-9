 ...
 */
function findIndexByProperty(arr, name1) {
  if (!Array.isArray(arr)) {
    return -1;
  }

  const a = arr.findIndex(({ name }) => name === name1);
  console.log(a);
...
function findLastEvenNumber(arr) {
  ...
  const a = arr.findLast((arr) => arr % 2 == 0);
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = b.filter((x) => x > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const a = arr.flat();
  const b = a.map((x) => x * x);
  const c = b.filter((x) => x > threshold);
  return c;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const a = arr.slice(-n);
  //   console.log(a);
  const sum = a.reduceRight((x, y) => x + y);
...
function wordLengths(arr) {
  ...
  const lengthsArray = arr.map((x) => x.length);
  return lengthsArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap((x) => Array(n).fill(x));
...
function findLongestWord(arr) {
  ...
  return arr.reduce((first, big) => {
    return big.length > first.length ? big : first;
  });
...
function findLastNegativeNumberIndex(arr) {
  ...
  const a = arr.findLastIndex((x) => x < 0);
  return a;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
